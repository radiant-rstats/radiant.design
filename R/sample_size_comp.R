...
sample_size_comp <- function(
 ...
 alternative = "two.sided"
  ...
  if (type == "mean") {
    ...
    n2 <- (1 + 1 / ratio) * (res$sd * (qnorm(1 - res$sig.level / adj) + qnorm(res$power)) / (res$delta)) ^ 2
    n2 <- ceiling(n2)
    n1 <- ceiling(ratio * n2)
  } else {
    ...
    if (is.null(ratio) || is.na(ratio)) ratio <- 1
    adj <- ifelse(alternative == "two.sided", 2, 1)
    n2 <- (res$p1 * (1 - res$p1) / ratio + res$p2 * (1 - res$p2)) * ((qnorm(1 - res$sig.level / adj) + qnorm(res$power)) / (res$p1 - res$p2)) ^ 2
    n2 <- ceiling(n2)
    n1 <- ceiling(ratio * n2)
  }
